...
1. 시연 화면은 `outputs/mai-report-rsna-demo/maiReport_RSNA시연_급성폐렴_프로토타입_v1.0.0.html`을 수정합니다.
...
`build-rsna.ps1`이 시연 화면을 그대로 `outputs/rsna-github-upload/index.html`로 내보냅니다. 이 파일만 별도 배포 저장소에 업로드합니다. 프로젝트 전체나 원자료는 올리지 않습니다. 현재 시연 HTML의 이미지는 문서 안에 포함되어 있습니다. 향후 외부 이미지 파일을 연결하면 배포 스크립트에도 해당 파일을 추가해야 합니다.
...
`outputs/mai-report-rsna-demo/gpt_maiReport_RSNA_google_sites_embed.html`을 삽입 코드에 붙여넣습니다. 이 사본은 구글 사이트의 이중 틀에 맞춰 두었습니다.
